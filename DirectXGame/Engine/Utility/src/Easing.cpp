...
Vector3 Slerp(Vector3 a, Vector3 b, float t) {
	if (t <= 0.0f) return a;
	if (t >= 1.0f) return b;
	float dotProduct = std::clamp(MyMath::dot(a, b), 0.0f, 1.0f);
...
uint32_t lerpColor(uint32_t start, uint32_t end, float t, EaseType type) {
	float easedT = Apply(t, type);
	uint8_t startR = (start >> 24) & 0xFF;
	uint8_t startG = (start >> 16) & 0xFF;
	uint8_t startB = (start >> 8) & 0xFF;
	uint8_t startA = start & 0xFF;

	uint8_t endR = (end >> 24) & 0xFF;
	uint8_t endG = (end >> 16) & 0xFF;
	uint8_t endB = (end >> 8) & 0xFF;
	uint8_t endA = end & 0xFF;

	uint8_t lerpR = static_cast<uint8_t>(startR + (endR - startR) * easedT);
	uint8_t lerpG = static_cast<uint8_t>(startG + (endG - startG) * easedT);
	uint8_t lerpB = static_cast<uint8_t>(startB + (endB - startB) * easedT);
	uint8_t lerpA = static_cast<uint8_t>(startA + (endA - startA) * easedT);

	return (lerpR << 24) | (lerpG << 16) | (lerpB << 8) | lerpA;
}
