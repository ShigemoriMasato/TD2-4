#include"OreBreakParticle.h"
#include"FpsCount.h"
#include"Utility/Easing.h"
#include"RandomGenerator.h"
#include <Common/DebugParam/GameParamEditor.h>

void OreBreakParticle::Initialize(DrawData drawData) {
...
void OreBreakParticle::Draw(Window* window, const Matrix4x4& vpMatrix) {

	// 描画
	instancingObject_->Draw(window, vpMatrix);
}

OreBreakParticle::ParticleData OreBreakParticle::MakeNewData(const Vector3& pos) {
	ParticleData data;
	...
	data.speed = RandomGenerator::Get(5.0f, 8.0f);
	data.rotSpeed = 12.0f;
	...
	data.velocity.y *= 1.5f;
	return data;
}

void OreBreakParticle::Move() {
	// 現在の数をリセット
	index_ = 0;
...
void OreBreakParticle::AddParticle(const Vector3& pos) {
	uint32_t spawnCount = 20;
...
void OreBreakParticle::RegisterDebugParam() {
...
void OreBreakParticle::ApplyDebugParam() {
