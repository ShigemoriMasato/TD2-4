...
void EnemyHP::Update(Matrix4x4 vpMatrix, float deltaTime, float currentHP, float maxHP, const Vector2& enemyScreenPos) {
	...
	hpBarAfter_.transform.position = {hpBarPos_.x - afterOffsetX, hpBarPos_.y, 0.0f};
	hpBarBG_.transform.position = {hpBarPos_.x, hpBarPos_.y, 0.0f};
...
void EnemyHP::Draw(CmdObj* cmdObj) {
	// 背景から前面の順に描画
...
void EnemyHP::InitializeRenderHPBar(SHEngine::ModelManager* modelManager, SHEngine::DrawDataManager* drawDataManager, std::unique_ptr<SHEngine::RenderObject>& render) {
	render->Initialize();
