...
void GameCamera::DrawImGui() {
	...
	if (ImGui::Checkbox("Manual Control", &manualControl_)) {
		needsUpdate = true;
	}
	...
	if (ImGui::SliderFloat("Distance Scale", &distanceScale_, minDistanceScale_, maxDistanceScale_)) {
		needsUpdate = true;
	}

	ImGui::Separator();

	if (ImGui::DragFloat3("Scale", &scale_.x, 0.1f)) {
		needsUpdate = true;
	}
	if (ImGui::DragFloat3("Rotation", &rotation_.x, 0.01f)) {
		needsUpdate = true;
	}
