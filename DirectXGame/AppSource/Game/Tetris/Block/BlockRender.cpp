...
void BlockRender::Initialize(std::map<int, std::pair<uint32_t, uint32_t>> colorMap, uint32_t fieldWidth, uint32_t fieldHeight, Camera* camera, const SHEngine::DrawData& drawData) {
	...
	//GPU準備
	blockObject_ = std::make_unique<SHEngine::RenderObject>("Tetris Block");
...
void BlockRender::Save() {
	binaryManager_->RegisterOutput(&holdBasePosition_);
	binaryManager_->RegisterOutput(&nextBasePosition_);
	binaryManager_->RegisterOutput(&nextGap_);

	for(const auto& [id, colors] : colorMap_) {
		binaryManager_->RegisterOutput(&id);
		binaryManager_->RegisterOutput(&colors.first);
		binaryManager_->RegisterOutput(&colors.second);
	}
...
void BlockRender::Load() {
	...
	holdBasePosition_ = binaryManager_->Reverse<Vector3>(data);
	nextBasePosition_ = binaryManager_->Reverse<Vector3>(data);
	nextGap_ = binaryManager_->Reverse<Vector3>(data);

	for(size_t i = 0; i < colorMap_.size(); ++i) {
		int id = binaryManager_->Reverse<int>(data);
		uint32_t color = binaryManager_->Reverse<uint32_t>(data);
		uint32_t outlineColor = binaryManager_->Reverse<uint32_t>(data);
		...
		colorMap_[id] = { color, outlineColor };
	}
}
